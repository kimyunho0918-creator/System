...
// 🌟 다중 구매 기능을 위해 대폭 변경된 상점 구매 로직 🌟
function buyItem(itemId) {
    const item = SHOP_ITEMS.find(i => i.id === itemId);

    // 슬롯머신(자율 충전)처럼 예외적인 상품은 기존 방식(1회 구매) 유지
    ...
    // 최대 살 수 있는 개수 계산 (가진 포인트 대비 개수 vs 남은 재고)
...
// 수량(qty) 파라미터를 추가하여 여러 개를 한 번에 처리하도록 업데이트
